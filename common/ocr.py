...
def ocr_img_rest_baidu(image,config):
    # 百度OCR API  ，在 https://cloud.baidu.com/product/ocr 上注册新建应用即可
    """ 你的 APPID AK SK """
    APP_ID = config.get('baidu_api','APP_ID')
    API_KEY = config.get('baidu_api','API_KEY')
    ...
    global rest_region
    # 切割题目+选项区域，左上角坐标和右下角坐标,自行测试分辨率
    rest_region = config.get("region", "rest_region").replace(' ','').split(',')
    rest_region = list(map(int, rest_region))
    region_im = image.crop((rest_region[0], rest_region[1], rest_region[2], rest_region[3]))
    print(region_im)
    # 转化为灰度图
    #region_im = region_im.convert('L')
...
def ocr_img_job_baidu(image,config):
    ...
    region_im = image.crop((job_region[0], job_region[1], job_region[2], job_region[3]))
    print(region_im)
    # 转化为灰度图
    #region_im = region_im.convert('L')
...
def ocr_img_region_baidu(image,config,region):
    APP_ID = config.get('baidu_api','APP_ID')
    API_KEY = config.get('baidu_api','API_KEY')
    SECRET_KEY = config.get('baidu_api','SECRET_KEY')
    ...
    job_region = region.replace(' ','').split(',')
    job_region = list(map(int, job_region))
    region_im = image.crop((job_region[0], job_region[1], job_region[2], job_region[3]))
